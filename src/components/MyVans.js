...
import { X, Edit2, Trash2, Eye, TrendingUp, Plus, Calendar, MapPin, DollarSign } from 'lucide-react';
...
export default function MyVans({ onClose }) {
  ...
  // Charger les vans de l'utilisateur
  useEffect(() => {
    const fetchMyVans = async () => {
  ...
  const handleDelete = async (van) => {
    try {
      ...
      if (van.images && van.images.length > 0) {
        const deletePromises = van.images.map(imageUrl => {
          try {
      ...
      const updatedVans = myVans.filter(v => v.id !== van.id);
      const totalViews = updatedVans.reduce((sum, v) => sum + (v.views || 0), 0);
      const avgPrice = updatedVans.length > 0 
        ? updatedVans.reduce((sum, v) => sum + v.price, 0) / updatedVans.length 
        : 0;
      
      setStats({
        totalVans: updatedVans.length,
        totalViews,
        avgPrice: Math.round(avgPrice)
      });
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-6xl w-full my-8 p-8 relative max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-3xl font-bold mb-6">Mes Vans 🚐</h2>
        
        {/* Statistiques */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-6 rounded-xl">
            <div className="flex items-center justify-between mb-2">
              <span className="text-emerald-600 font-semibold">Total Vans</span>
        ...
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 font-semibold">Chargement de vos vans...</p>
          </div>
        )}
        ...
        {!loading && myVans.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-xl font-bold text-gray-400 mb-2">Aucun van listé</p>
            <p className="text-gray-500 mb-4">Commencez par ajouter votre premier van !</p>
            <button 
              onClick={onClose}
        ...
        {!loading && myVans.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {myVans.map(van => (
              <div key={van.id} className="bg-white border-2 border-gray-200 rounded-xl overflow-hidden hover:shadow-xl transition">
