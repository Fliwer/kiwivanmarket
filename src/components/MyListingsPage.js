            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-white/80 text-sm">{t('my_listings.manage')}</p>
                </div>
              </div>
              <Link
                to="/sell"
              ...
              >
                <Plus size={20} />
                <span className="hidden sm:inline">{t('my_listings.add_new')}</span>
              </Link>
            </div>
          </div>
        </header>
        ...
        <main className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-xl p-4 shadow-sm text-center">
              ...
              <p className="text-sm text-gray-500">{t('my_listings.stats_listings')}</p>
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm text-center">
              <Eye className="w-6 h-6 text-blue-600 mx-auto mb-2" />
              <p className="text-2xl font-bold text-gray-900">{stats.totalViews}</p>
              <p className="text-sm text-gray-500">{t('my_listings.stats_views')}</p>
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm text-center">
              <DollarSign className="w-6 h-6 text-green-600 mx-auto mb-2" />
              <p className="text-2xl font-bold text-gray-900">${stats.avgPrice.toLocaleString()}</p>
              <p className="text-sm text-gray-500">{t('my_listings.stats_price')}</p>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-sm font-medium transition ${viewMode === 'all'
                      ? 'bg-yellow-500 text-white'
                      : 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200'
                      }`}
                  >
                  ...
                  <div className="sm:w-48 h-40 sm:h-auto relative flex-shrink-0">
                    ...
                    {van.status === 'sold' && (
                      <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                        <span className="text-white font-bold text-lg">{t('my_listings.status_sold')}</span>
                      </div>
                    )}
                    {van.status === 'paused' && (
                      <div className="absolute inset-0 bg-yellow-500/80 flex items-center justify-center">
                        <span className="text-white font-bold text-lg">{t('my_listings.status_paused')}</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6">
              <div className="flex items-center gap-3 text-red-600 mb-4">
                ...
                <h3 className="text-xl font-bold">{t('my_listings.delete_confirm_title')}</h3>
              </div>
              <p className="text-gray-600 mb-6">
                {t('my_listings.delete_confirm_desc', { title: showDeleteConfirm.title })}
              </p>
