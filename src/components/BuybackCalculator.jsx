          ...
          <div className="lg:col-span-12 xl:col-span-5">
            <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-slate-200/40">
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Self-Containment (NZ Standards)</label>
                  <div className="grid grid-cols-3 gap-2">
                    {Object.entries(CONFIG.scStatus).map(([key, data]) => (
                      <button
                        key={key}
                        onClick={() => handleUpdate(() => setSc(key))}
                        className={`p-3 rounded-xl border-2 text-[10px] font-bold transition-all ${sc === key ? 'bg-emerald-600 border-emerald-600 text-white shadow-md' : 'bg-slate-50 border-slate-50 text-slate-500 hover:bg-slate-100'}`}
                      >
                        {data.label.split('(')[0]}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider text-ellipsis overflow-hidden whitespace-nowrap">Service History</label>
                    <select
                      value={history}
                      onChange={(e) => handleUpdate(() => setHistory(e.target.value))}
                      className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-3 outline-none focus:border-emerald-500/30 font-bold text-sm text-slate-700"
                    >
                  ...
                  <div className="space-y-3">
                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Equipment</label>
                    <select
                      value={equip}
                ...
                <div className="space-y-3">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Market Context & Timing</label>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <p className="text-[9px] font-black text-slate-300 uppercase">Season</p>
                    ...
                    <div className="space-y-2">
                      <p className="text-[9px] font-black text-slate-300 uppercase">Trend</p>
                      <select
                        value={trend}
                        onChange={(e) => handleUpdate(() => setTrend(e.target.value))}
                        className="w-full bg-slate-50 border-none rounded-xl px-3 py-2 outline-none font-bold text-[10px] text-slate-600"
                      >
